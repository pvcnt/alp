...
import fr.cnrs.liris.util.{Distance, TriState}
...
object Converter {
  ...
  register(new Converter[JdkDuration] {
    override def convert(input: String): JdkDuration = {
      JdkDuration.ofNanos(TwitterDuration.parse(input).inNanoseconds)
    }
